...
# date: 2018-05-27
# file: 01_analyze_freeDiffusion.py
##########################################################################################    
...
def Plot(titlestr, X, MSD, outname, outdir, grid = True,
    ...
    major_x_ticks = np.arange(-12.0, 21.0, 4.0)
    minor_x_ticks = np.arange(-12.0, 21.0, 2.0)
    ...
    xVals = np.linspace(-15.0, 15.0, 2000)
    ...
    for i in range(nSamples):
        ...
        ax1.plot(xVals, FPE_free(xVals, x0 = x0Value, t = sampleTimes[i]),
                 color = RGBcolorValue,
                 lw = 1.0)
    ...
    ax2 = f.add_axes([0.25, 0.40, 0.22, 0.23])
    ...
    major_y_ticks = np.arange(1.0, 2.55, 0.5)
    minor_y_ticks = np.arange(-1.0, 2.55, 0.1)
    ...
    ax2.plot([-10.0, 20.0], [x0Value, x0Value],
            lw = 0.75,
            alpha = 1.0,
            color = '#666666',
            zorder = 1,
    ...
    major_y_ticks = np.arange(0.0, 25.1, 10.0)
    minor_y_ticks = np.arange(0.0, 25.1, 2.0)
    ...
    xVals = np.linspace(-5.0, 15.0, 300)
    yVals = [2.0 * x for x in xVals]
    ...
    leg = ax3.legend(bbox_to_anchor = [-0.02, 1.02],
                     loc = 'upper left',
                     fontsize = 4.0,
                     handlelength = 2.0,
                     scatterpoints = 1,
    ...
    ax3.xaxis.labelpad = 0.0
    ax3.yaxis.labelpad = 1.0
                  
    ax3.set_xlim(-0.5, 11.5)
    ax3.set_ylim(-1.0, 21.5)
...
if __name__ == '__main__':
    ...
    filename = '01_freeDiffusion_m_100000_dt_1e-03.txt'
